...
func solve(countCuts int, firstDeck, secondDeck string) string {
	...
	if len(firstDeck) != len(secondDeck) || countCuts == 0 {
		return "NO"
	}
	...
	if !strings.Contains(firstDeck+" "+firstDeck, secondDeck) || (countCuts == 1 && firstDeck == secondDeck) {
		return "NO"
	}
